...
execute if entity @s[tag=sp_here] unless entity @e[type=marker,tag=tp_spawner,tag=!itsmemario,tag=!donebefore,distance=..15,limit=1,sort=random] run summon item ~0.5 ~0.5 ~0.5 {NoGravity:1b,Glowing:1b,Item:{id:"minecraft:diamond",Count:1b}}
